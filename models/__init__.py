...
from .MultiModal import ConcatMultiModalNet, GateKroneckerFusion,SelfAttentionMultiModalNet, TransformerMultiModalNet, \
    MultiHeadAttentionMultiModalNet, CrossAttentionMultiModalNet
from .SNP import SimpleLinear, DoubleLayerSimpleMLP, ResMLP, CapsuleNet, DeepExGRS,GeneNet,LassoModel,CNN1D
from plink.genome_local_net import DeepcvGRS
from models.InteractionModel import InteractionNN,PermutationNN

class SNPNet(nn.Module):
    def __init__(self, snp_number: int):
        super(SNPNet, self).__init__()
        self.snp_number = snp_number
        #self.snp_model =GeneNet(snp_number)
        #self.snp_model = SimpleLinear(snp_number)
        #self.snp_model = DoubleLayerSimpleMLP(snp_number)
        self.snp_model = DeepcvGRS(snp_number)
        #self.snp_model = LassoModel(snp_number)
        #self.snp_model = CNN1D()
...
class InteractionNet(nn.Module):
    def __init__(self, gene_block, Predictor):
        super(InteractionNet, self).__init__()
        self.snp_model = InteractionNN(gene_block, Predictor)
        #self.snp_model =PermutationNN(gene_block, Predictor)
    def forward(self, snps):
        x = self.snp_model(snps)
        return x
...
class ImageNet(nn.Module):
    def __init__(self):
        ...
        self.image_model = convnext_tiny(num_classes=1)#weights=False0.89  #num_classes=1 0.91  #convnext_tiny()0.89  #weights=True0.99
...
class SNPImageNet(nn.Module):
    def __init__(self, snp_number: int):
        ...
        self.multi_modal_model = GateKroneckerFusion(snp_number)
...
nets = {
    ...
    f'InteractionNet':InteractionNet,
}
