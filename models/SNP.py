...
class GeneNet(nn.Module):
    def __init__(self, snp_number):
        super().__init__()
        self.snp_number=snp_number
        self.linear1 = nn.Linear(snp_number, 256)
    ...
    def forward(self, snps):
        # print(snps)
        #x=self.dropout1(self.gelu1(self.BN1(self.linear1(snps))))
        #x = self.dropout2(self.gelu2(self.BN2(self.linear2(snps))))
        x=self.gelu1(self.BN1(self.linear1(snps)))
        x=self.gelu2(self.BN2(self.linear2(x)))
        x=self.linear3(x)
        return x
...
class CNN1D(nn.Module):
    def __init__(self):
        ...
        self.model1 = nn.Sequential(
            ...
            nn.ReLU(),  
            nn.MaxPool1d(kernel_size=2),  # 输出大小：torch.Size([batch_size, 16, (input_length-2)/2])
            nn.Conv1d(16, 32, kernel_size=2),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=4)   # 输出大小：torch.Size([batch_size, 32, (input_length-2)/2-1])
        )

        self.flattened_size = self._get_flattened_size(input_length=179712)
        
        self.model2 = nn.Sequential(
            nn.Linear(in_features=self.flattened_size, out_features=1, bias=True)
        )

    def _get_flattened_size(self, input_length):

        length_after_conv1 = (input_length - 2) // 2
        length_after_conv2 = (length_after_conv1 - 2) // 4
        return 32 * length_after_conv2
...
class LassoModel(nn.Module):
    def __init__(self,snp_number, l1_penalty=0.1):
        super(LassoModel, self).__init__()
        self.linear = nn.Linear(snp_number, 1, bias=True)
        self.l1_penalty = l1_penalty

    def forward(self, x):
        return self.linear(x)
        
    def lasso_loss(self, outputs, labels):
