...
def PlinkQC( raw_data_path,
             ...
             extract_snp_file=""):
    if extract_snp_file:
        input_path=PlinkExtractAlleles(raw_data_path, extract_snp_file,extract_snp_output_path) #克隆任务是指创建一个与原始任务相同的新任务实例。
    else:
...
if __name__ =="__main__":
    ...
    plink_path=r"/pub/data/gaoss/data/MAGIC/no-delect-no-rs/MAGIC_prs/75_HM_gene/valid/"
    one_hot_path=r"/pub/data/gaoss/New_Multi/code/MAGIC_prs_one_hot/75_HM_gene/valid-one-hot/"
    with open(gene_list, "r") as f:
        genes = f.read().splitlines()
    i=0
    for gene in tqdm.tqdm(genes):
        i+=1
        path=os.path.join(plink_path,gene)
        outpath=os.path.join(one_hot_path,gene)
        mk_dir(outpath)
        raw_data_path = path # help="Path to raw data folder to be processed (containing data.bed, data.fam,data.bim)
        extract_snp_output_path = None
        qc_output_path = None
        OneHotSNPs_output_path = outpath
