...
class ImageDataset(Dataset):
    ...
    def set_label_data_dict(self,model="single"):
        print(model)
        label_data_dict = {}
        label_name='label'
        if model =='single':
            ...
            print("###################Enter multimodal labeling based on genetics######################")
            for index, row in self.label_df.iterrows():
                if row[f'OS_{self.label_data_label_field_name}']==1 and row[f'OD_{self.label_data_label_field_name}']==1:
                    label_data_dict[f"{row[self.label_data_id_field_name]}_OS"]=1
                    label_data_dict[f"{row[self.label_data_id_field_name]}_OD"]=1
                elif row[f'OS_{self.label_data_label_field_name}']==0 and row[f'OD_{self.label_data_label_field_name}']==0:
                    label_data_dict[f"{row[self.label_data_id_field_name]}_OS"]=0
                    label_data_dict[f"{row[self.label_data_id_field_name]}_OD"]=0
                elif row[f'OS_{self.label_data_label_field_name}']==row[label_name]:
                    label_data_dict[f"{row[self.label_data_id_field_name]}_OS"]=row[label_name]
                else:
                    label_data_dict[f"{row[self.label_data_id_field_name]}_OD"]=row[label_name]
...
class SNPImageDataset(SNPDataset, ImageDataset):
    def __init__(
        ...
        self.label_data_dict = self.set_label_data_dict(model="multi")
        self.image_data_file_names = self.filter_image_data_file_names()
        # self.image_data_file_names = self.image_data_balance()
        # print(f'length of dataset after balance: {len(self.image_data_file_names)}')
        self.use_eye_side = use_eye_side
