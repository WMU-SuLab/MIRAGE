...
with open("/pub/data/gaoss/New_Multi/code/MAGIC_prs_20/75_HM/HM_gene_size.txt", newline='') as f:
    reader = csv.reader(f)
    tmp = list(reader)
...
## Load the data
x_path = "/pub/data/gaoss/New_Multi/code/GWAS_NN/train/train.csv"
y_path = "/pub/data/gaoss/New_Multi/code/GWAS_NN/train/labels.csv"
...
def one_permutation(index):
    ...
    Y_null = (Y_null >= 0.5).float()
    x_null, x_test_null, y_null , y_test_null, X_processed_null, Y_processed_null = preprocessing_permutation(X_processed.numpy(), Y_null.cpu().numpy())
    ...
    idx = idx.to(x_test.device)
    gene_test, _ = GINN_null.model.encoder(x_test_null[idx].to(device)); 
    gene_test.detach_()
    baseline = torch.mean(gene_test, dim = 0).view(1,-1).to(device)
    print(baseline)
    Shapely_NN = GlobalSIS(GINN_null.model.predictor, gene_test, baseline)#GlobalSIS_NN, topGlobalSIS_NN,
    np.savetxt("/pub/data/gaoss/New_Multi/code/GWAS_NN/result1/"+name+'.csv', Shapely_NN, delimiter=",")
