...
with open("/pub/data/gaoss/New_Multi/code/GWAS_NN/data/snps_size.txt", newline='') as f:
    reader = csv.reader(f)
    tmp = list(reader)
...
## Load the data
x_path = "/pub/data/gaoss/New_Multi/code/GWAS_NN/data/genotype.csv"
y_path = "/pub/data/gaoss/New_Multi/code/GWAS_NN/data/phenotype.csv"
x, x_test, y, y_test = load_data(x_path, y_path); batch_size = int(x.shape[0] / 100)


## Gene interaction NN training
encoder = Encoder(gene_size, device = device)
predictor = Predictor(gene_size)
...
np.savetxt("/pub/data/gaoss/New_Multi/code/GWAS_NN/result/"+name+'.csv', Shapely_NN, delimiter=",")
