...
def inputGradient(deepcvGRS, x):
    output = deepcvGRS(x)
    first = torch.autograd.grad(output, x)
    return first[0].view(-1)

def inputHessian(deepcvGRS, x, device):
    Hessian = []
    output = deepcvGRS(x)
...
def IntegratedHessian(deepcvGRS, xi, baseline, device):
    ...
    for p in range(m):
        for l in range(k):
            ...
            PathHessian = PathHessian + (l+1) / k * (p+1) / m * torch.tensor(inputHessian(deepcvGRS, x_eva, device)).to(device) / (k * m)
            PathGradient = PathGradient + inputGradient(deepcvGRS, x_eva) / (k * m)
    ItgHessian = PathHessian * Diff2 + torch.diag(PathGradient * diff.view(-1))
    return ItgHessian

def GlobalIH(deepcvGRS, X, baseline, device):
    num_individual, num_gene = X.shape
    Hessian = torch.zeros([num_gene, num_gene]).to(device)
    
    for i in range(num_individual):
        x = X[i].clone().view(1,-1)
        Hessian = Hessian + torch.abs(IntegratedHessian(deepcvGRS, x, baseline, device))
...
def delta_main(deepcvGRS, xi, baseline, main_index):
    Ti = copy_values(xi, baseline, main_index).view(1,-1)
    T = copy_values(xi, baseline, []).view(1,-1)
    output_Ti= deepcvGRS(Ti); output_T = deepcvGRS(T); 
    return output_Ti.item() - output_T.item()

def deltaF(deepcvGRS, xi, baseline, interaction, T):
    ...
    output_Tij = deepcvGRS(Tij); output_Ti = deepcvGRS(Ti); output_Tj = deepcvGRS(Tj); output_T= deepcvGRS(T); 
    return output_Tij.item() - output_Ti.item() - output_Tj.item() + output_T.item()

def ShapleyValue(deepcvGRS, xi, baseline):
    num_gene = xi.shape[0]
    shapleyvalue = np.zeros([num_gene])
    for i in range(num_gene):
        shapleyvalue[i] = delta_main(deepcvGRS, xi, baseline, [i])
    return shapleyvalue

def ShapleyIS(deepcvGRS, xi, baseline, num_permutation):
    num_gene = xi.shape[0]
    print("num-gene",num_gene)
    SHAPLEYIS = np.zeros([num_gene, num_gene])
    for m in range(num_permutation):
        perm = list(np.random.permutation(num_gene))
        print("perm",perm)
        T = []
        shapleyis = np.zeros([num_gene, num_gene])
        for i in range(len(perm)):
            ...
            for j in range(i+1, len(perm)):
                shapleyis[perm[i]][perm[j]] = deltaF(deepcvGRS, xi, baseline, [perm[i],perm[j]], T)
        SHAPLEYIS = SHAPLEYIS + shapleyis

    SHAPLEYIS = (SHAPLEYIS + SHAPLEYIS.T) / num_permutation
    SHAPLEYIS = SHAPLEYIS +  np.diag(ShapleyValue(deepcvGRS, xi, baseline)) 
    return SHAPLEYIS

def GlobalSIS(deepcvGRS, X, baseline, num_permutation = 1):
    all_shapley_matrices = [] 
    num_individual, num_gene = X.shape
    Shapely = np.zeros([num_gene, num_gene])    
    for i in tqdm.tqdm(range(num_individual)):
        x = X[i].clone()
        print("x",x)
        shapley_matrix = ShapleyIS(deepcvGRS, x, baseline.view(-1), num_permutation)
        all_shapley_matrices.append(shapley_matrix)
        Shapely = Shapely + abs(shapley_matrix)
    Shapely = Shapely / num_individual
    all_shapley_matrices = np.array(all_shapley_matrices)
    #GlobalSIS, topGlobalSIS = matric2dic(Shapely, 5)
    return  Shapely ,all_shapley_matrices#GlobalSIS, topGlobalSIS,