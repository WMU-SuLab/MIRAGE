...
from shap_value import SNP_Shap_Value,Image_Shap_Value
...
def main(test_dir_prefix,model_name,dataset_dir_path,wts_path,snp_number,dataset_in_memory=True, persistent_workers=True,
    ...
    if model_name=="SNPNet":
        print("Enter calculation SNP shap")
        SNP_Shap_Value(net,data_loaders['train'], data_loaders['valid'], device,test_dir_prefix,valid=True)
    if model_name=="ImageNet":
        print("Enter calculation Image shap")
        Image_Shap_Value(net, data_loaders['valid'], data_loaders['test'], device,test_dir_prefix,valid=True)
    '''
